%% Main File for running MPC with linearized dynamics
...
% Weight Matrices
Q = diag([1, 0, 0, 0, 1, 0]); % [vx; vy; wz; s; e_lat; e_psi]
R = diag([1,10]); % Need a way to smooth the steering angle
...
s_predict = 0:dt:dt*N;
x_opt = [x0(1)*ones(1,N+1); x0(2)*ones(1,N+1); x0(3)*ones(1,N+1); ...
         s_predict; x0(5)*ones(1,N+1); x0(6)*ones(1,N+1)];
...
while (x_curv(4) <= classTrack.trackLength)
