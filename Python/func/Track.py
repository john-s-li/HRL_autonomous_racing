...
class Map():
    ...
    def __init__(self, width):
        ...
        # The elements of spec are defined as [arclength, radius]
        # A negative radius denotes clockwise motion
        # A radius of 0 = straight line segment
        spec = np.array([[3, 0], 
                         [np.pi/2 *1, -1], 
                         [2, 0], 
                         [np.pi/2*1, -1], 
        ...
        # Now given the above segments we compute the (x, y) points of the track and the angle of 
        # the tangent vector (psi) at these points. For each segment we compute the (x, y, psi) 
        # coordinate at the last point of the segment. Furthermore, we compute also the cumulative 
        # s at the starting point of the segment at signed curvature
    ...
    def getGlobalPosition(self, s, ey):
        ...
        index = np.all([[s >= PointAndTangent[:, 3]], [s < PointAndTangent[:, 3] + PointAndTangent[:, 4] + s_tolerance]], axis=0)
